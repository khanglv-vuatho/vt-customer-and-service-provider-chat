...
const fetchMessage = async ({ orderId, worker_id, page, limit = 10 }: { orderId: number; worker_id?: number; page: number; limit?: number }) => {
  ...
  const response = await instance.get(endpoint, { params: { ...params, page, limit } })

import { DefaultModal } from '@/components/Modal'
import { handleAddLangInUrl } from '@/utils'
import { Button } from '@nextui-org/react'
import React, { useState } from 'react'
import { useNavigate } from 'react-router-dom'

const Redirect = () => {
  const [isOpen, setIsOpen] = useState(true)
  const navigate = useNavigate()

  const handleRedirectWorker = () => {
    navigate(
      handleAddLangInUrl({
        lang: 'vi',
        mainUrl:
          '/chat?currentId=429&orderId=3310&avatarUrl=https://www.google.com/url?sa=i&url=https%3A%2F%2Fphongvu.vn%2Fcong-nghe%2Ftop-10-trang-web-tai-hinh-nen-dep-nhat-2020-phan-1%2F&psig=AOvVaw2MhrDPmAdrTefs_VZoaUGs&ust=1722498306359000&source=images&cd=vfe&opi=89978449&ved=0CBEQjRxqFwoTCNiei6_k0IcDFQAAAAAdAAAAABAR',
        token:
          'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..JBA8db00sY4Qk5KoBrN72zY6irfqJfymmx5vXlrug7U'
      })
    )
  }
  const handleRedirectWClient = () => {
    navigate(
      handleAddLangInUrl({
        lang: 'vi',
        mainUrl:
          '/chat?currentId=570&orderId=3310&worker_id=429&avatarUrl=https://www.google.com/url?sa=i&url=https%3A%2F%2Fdidongviet.vn%2Fdchannel%2Fhinh-nen-iphone-dep-3d%2F&psig=AOvVaw2MhrDPmAdrTefs_VZoaUGs&ust=1722498306359000&source=images&cd=vfe&opi=89978449&ved=0CBEQjRxqFwoTCNiei6_k0IcDFQAAAAAdAAAAABAE',
        token:
          'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.._X3tn1J1flDRnZsVkVY9BMnFis3XU4avJR7RScPohAs'
      })
    )
  }

  const handleRedirect = (role: string) => {
    setIsOpen(false)
    role == 'client' ? handleRedirectWClient() : handleRedirectWorker()
  }

  return (
    <div className='h-dvh'>
      <DefaultModal isOpen={isOpen} onOpenChange={() => {}}>
        <Button onClick={() => handleRedirect('client')} className='bg-primary-blue text-white'>
          Khách
        </Button>
        <Button onClick={() => handleRedirect('worker')} className='bg-primary-yellow text-white'>
          Thợ
        </Button>
      </DefaultModal>
    </div>
  )
}

export default Redirect

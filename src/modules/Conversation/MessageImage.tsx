...
const MessageImage = ({ url }: { url: string }) => {
  ...
  const handleAddResizeImage = (src: string) => {
    return src.includes('blob') ? `${src}?width=320&height=320` : `${src}?width=10&height=10`
  }
  ...
  return (
    <>
      <div onClick={isLoading ? undefined : handleZoomImage} className='max-w-[60%] overflow-hidden rounded-md'>
        ...
        <Image removeWrapper src={handleAddResizeImage(url)} alt={url} className={`size-full object-cover ${!isLoading ? 'block' : 'hidden'}`} onLoad={handleOnLoadImage} />
      </div>
      <DefaultModal className='z-[200] h-auto max-h-[96dvh]' isOpen={isOpenModalImage} onOpenChange={handleZoomImage}>
        <div className='flex h-full flex-col items-center'>
          ...
          <div className='flex max-h-[700px] w-full overflow-hidden rounded-lg px-16'>
            ...
            <Image removeWrapper src={handleAddResizeImage(url)} alt={url} className={`size-full object-cover ${!isLoadingInModal ? 'block' : 'hidden'}`} onLoad={handleOnLoadImageModal} />
